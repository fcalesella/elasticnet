...
Note that to run the script and the function a working MATLAB is needed, whereas for the standalone app only a MATLAB runtime is required.

### 3.1 Script <a name="script"></a>
To run the ```elasticnet``` script some parameters have to be set at the beginning of the script. Here an explanation of the meaning of each parameter is provided:
